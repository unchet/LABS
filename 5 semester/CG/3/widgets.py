...
def change_intensivity(value):
    global intensivity
    intensivity = value


def change_diffuse(value):
    global diffuse
    diffuse = value
...
def make_window():
    ...
    scale_intensivity = tk.Scale(window, from_=0, to=1, orient=HORIZONTAL, resolution=0.1, command=change_intensivity)
    label_intensivity = tk.Label(window, text="Интенсивность фонового освещения")
    label_intensivity.grid(row=8, column=0)
    scale_intensivity.grid(row=8, column=1)

    scale_diffuse = tk.Scale(window, from_=0, to=1, orient=HORIZONTAL, resolution=0.1, command=change_diffuse)
    label_diffuse = tk.Label(window, text="Интенсивность освещения от источника")
    label_diffuse.grid(row=9, column=0)
    scale_diffuse.grid(row=9, column=1)
